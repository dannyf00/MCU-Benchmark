...
uint32_t benchmark(void (* f_ptr)(void)) {
	uint32_t clock_start, clock_end;

	clock_start=benchmark_clock();				//take the starting clock
	//do {f_ptr();} while (count--);			//run the benchmark
	//while (count--) f_ptr();
	f_ptr();
	clock_end=benchmark_clock();					//take the ending clock
